...
from download_url_pretrained import download_pretrained_model
class FRCNN(nn.Module):

    def __init__(self, pretrained=False, num_classes=81):
        super().__init__()
        ...
        if pretrained:
            pth_name = 'frcnn.best.pth.tar'
            download_pretrained_model(pth_name, '14ObbUCsP7DsDd2kMcnIxJRpnAFsT-9Lj')
            checkpoint = torch.load(
                    f=os.path.join(torch.hub.get_dir(), 'checkpoints', pth_name), map_location=torch.device('cpu'))
    ...
    def predict(self, x, threshold):
        ...
        bbox, label, score = self._suppress(pred_bbox, pred_cls, threshold)
        return bbox, label, score

    def _suppress(self, raw_cls_bbox, raw_prob, threshold):
        bbox = list()
        label = list()
        score = list()
