...
from datasets.transforms_ import RandomSizeCrop, Resize, Compose
...
def load_mosaic(image_id, len_of_dataset, size, _load_image, _load_anno, _parse, image, boxes, labels):
    ...
    pre_mosaic_transform = Compose([
        Resize(size, max_size=1333),
        RandomSizeCrop(384, size),
        Resize((size, size)),
    ])
