...
def visualize_detection_result(img_pil, boxes, labels, scores):
    ...
    # 2. resize the images (the range of boxes is 0 to 1)
    img_height, img_width = x_img.shape[:2]
...
def demo(img_path, threshold):
    ...
    model = FRCNN(pretrained=True, num_classes=81).cuda()
    model.eval()
    with torch.no_grad():
        pred_boxes, pred_labels, pred_scores = model.predict(batch_img, threshold)
...
if __name__ == '__main__':
    ...
    # classes 81
    coco_labels_list = ['person', 'bicycle', 'car', 'motorcycle', 'airplane',
    ...
    coco_labels_map = {k: v for v, k in enumerate(coco_labels_list)}
    np.random.seed(1)
    coco_colors_array = np.random.randint(256, size=(81, 3)) / 255

    # demo
    # demo('./soccer.png', threshold=0.9)
    demo('./tennis.jpg', threshold=0.8)